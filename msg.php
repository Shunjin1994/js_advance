...
//================================
// GETパラメータを取得
$id = (!empty($_GET['b_id'])) ? $_GET['b_id'] : '';
// DBから掲示板とメッセージデータを取得
$viewData = getMsgsAndBord($id);
...
if(!empty($_POST)){
  ...
  if(empty($err_msg)){
    ...
    try {
      ...
      $data = array(':b_id' => $id, ':send_date' => date('Y-m-d H:i:s'), ':to_user' => $partnerUserId, ':from_user' => $_SESSION['user_id'], ':msg' => $msg, ':date' => date('Y-m-d H:i:s'));
      // クエリ実行
      ...
      if($stmt){
        ...
        debug('連絡掲示板へ遷移します。');
        header("Location: " . $_SERVER['PHP_SELF'] .'?b_id='.$id); //自分自身に遷移する
      }
